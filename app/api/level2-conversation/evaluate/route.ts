...
const vocabularyIds: string[] = conversationRoom.targetVocabulary.map((item) => item.id);
...
export async function POST(request: Request) {
  try {
    ...
    const report = JSON.parse(getOpenAIResponseText(payload)) as Record<string, unknown>;
    const dimensions = conversationRoom.rubric.dimensions.map((definition) => {
      const candidate = Array.isArray(report.dimensions)
        ? report.dimensions.map(asRecord).find((item) => item?.id === definition.id)
        : null;
      return {
        id: definition.id,
        max: definition.max,
    ...
    const validVocabulary = Array.isArray(report.usedVocabulary)
      ? report.usedVocabulary.flatMap((value) => {
        const item = asRecord(value);
        if (!item || typeof item.id !== 'string' || !vocabularyIds.includes(item.id)) return [];
        return [{
          id: item.id,
          heardForm: typeof item.heardForm === 'string' ? item.heardForm.slice(0, 200) : '',
          evidence: typeof item.evidence === 'string' ? item.evidence.slice(0, 500) : '',
          correct: item.correct === true,
        }];
      }).slice(0, 16)
      : [];
    const correctVocabularyCount = new Set(validVocabulary.filter((item) => item.correct).map((item) => item.id)).size;
