...
import { getOpenAIResponseText } from '../../../../lib/openai-response';
import { countExamWords, determineExamStatus, finalExamPrivate, scoreObjectiveAnswers } from '../../../../lib/level3-exam-server';
import { productionEvaluatorPrompt } from '../../../../data/level3/final-exam-prompts';

export const runtime = 'nodejs';

type TranscriptEntry = { role: 'learner' | 'examiner'; text: string };
type ProductionEvaluationPayload = Record<string, unknown> & {
...
function schemaFor(kind: 'writing' | 'speaking') {
  const dimensions = kind === 'writing'
    ? { task: 4, organization: 3, language: 3, grammar: 3, mechanics: 2 }
    : { task: 4, coherence: 3, expressions: 3, grammar: 3, intelligibility: 2 };
  return {
...
async function evaluateProduction(kind: 'writing' | 'speaking', content: string, userId: string) {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      model: process.env.OPENAI_EVALUATION_MODEL || 'gpt-4o-mini', instructions: 'قيّم فقط وفق السلم. أعد JSON صالحًا مطابقًا للمخطط.',
  ...
  const evaluation = JSON.parse(getOpenAIResponseText(payload)) as ProductionEvaluationPayload;
  const score = evaluation.evaluationStatus === 'complete'
    ...
    : 0;
  return { ...evaluation, score: Math.min(15, score), corrections: (evaluation.corrections || []).slice(0, 3) };
}
...
export async function POST(request: Request) {
  try {
    ...
    const { data: attempt } = await supabase.from('level3_exam_attempts').select('*').eq('id', body.attemptId).eq('user_id', userId).maybeSingle();
    ...
    if (countExamWords(writingText) < 120 || countExamWords(writingText) > 160) return Response.json({ error: 'WRITING_WORD_COUNT' }, { status: 409 });
    if (!writingText.trim() || speakingTranscript.filter((entry) => entry.role === 'learner').length < 2) return Response.json({ error: 'PRODUCTION_SECTIONS_INCOMPLETE' }, { status: 409 });
    if (attempt) {
    ...
    let writingEvaluation: Awaited<ReturnType<typeof evaluateProduction>>;
    let speakingEvaluation: Awaited<ReturnType<typeof evaluateProduction>>;
    try {
      writingEvaluation = attempt?.status === 'production_retake' && Number(previousWriting?.score) >= 8 ? previousWriting : await evaluateProduction('writing', writingText, userId);
      speakingEvaluation = attempt?.status === 'production_retake' && Number(previousSpeaking?.score) >= 8 ? previousSpeaking : await evaluateProduction('speaking', speakingTranscript.map((entry) => `${entry.role === 'learner' ? 'الطالب' : 'الممتحن'}: ${entry.text}`).join('\n'), userId);
    } catch (error) {
      await supabase.from('level3_exam_attempts').update({ status: 'evaluation_pending', answers, writing_text: writingText, speaking_transcript: speakingTranscript, updated_at: new Date().toISOString() }).eq('id', attempt.id).eq('user_id', userId);
    ...
    return Response.json({
      resultStatus, totalScore, bestScore, passScore: 70, sectionScores,
    ...
    console.error('Level 3 exam submit error', error instanceof Error ? error.message : 'unknown');
