...
import { speakingExaminerPrompt } from '../../../../data/level2/final-exam-prompts';
...
export async function POST(request: Request) {
  try {
    ...
    const { data: attempt } = await supabase.from('level2_exam_attempts').select('id,status,expires_at,submitted_sections').eq('id', attemptId).eq('user_id', user.id).maybeSingle();
    ...
    const session = {
      ...
      audio: {
        input: {
          ...
          transcription: { model: process.env.OPENAI_TRANSCRIPTION_MODEL || 'gpt-realtime-whisper', language: 'ar' },
    ...
    if (!upstream.ok) {
      console.error('A2 exam realtime session failed', upstream.status, payload.slice(0, 250));
    ...
    console.error('A2 exam realtime route error', error instanceof Error ? error.message : 'unknown');
