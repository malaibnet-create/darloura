...
import { determineExamStatus, finalExamPrivate, scoreObjectiveAnswers } from '../../../../lib/level2-exam-server';
...
export async function POST(request: Request) {
  try {
    ...
    const { data: attempt } = await supabase.from('level2_exam_attempts').select('*').eq('id', body.attemptId).eq('user_id', userId).maybeSingle();
    ...
    try {
      ...
      await supabase.from('level2_exam_attempts').update({ status: 'evaluation_pending', answers, writing_text: writingText, speaking_transcript: speakingTranscript, updated_at: new Date().toISOString() }).eq('id', attempt.id).eq('user_id', userId);
