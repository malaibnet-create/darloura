...
function grade(item: PrivateItem, response: unknown) {
  ...
  if (['singleChoice', 'audioChoice', 'contextMeaning', 'fillBlank', 'audioComprehension', 'oddOneOut'].includes(item.type)) {
    ...
    const accepted = item.acceptedAnswers
      ?? (typeof item.modelAnswer === 'string' ? [item.modelAnswer] : []);
    correct = accepted.some((acceptedAnswer) => sameText(response, acceptedAnswer));
  } else if (item.type === 'match') {
    const submitted = response && typeof response === 'object' ? response as Record<string, string> : {};
    correct = (item.pairs ?? []).every((pair) => submitted[pair[0]] === pair[1]);
  } else if (item.type === 'familySort') {
    const submitted = response && typeof response === 'object' ? response as Record<string, string> : {};
    correct = Object.entries(item.groups ?? {}).every(([group, terms]) => terms.every((term) => submitted[term] === group));
    ...
    const task = words >= (item.minimumWords ?? 60) && words <= (item.maximumWords ?? 80) ? 3 : words >= 45 ? 2 : words >= 20 ? 1 : 0;
    ...
    correct = task + vocabulary + clarity + accuracy >= 7 && detected.length >= (item.minimumTargetTerms ?? 6);
