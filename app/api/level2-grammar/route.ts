...
type PrivateItem = Record<string, unknown> & {
  id: string;
  type: string;
  promptAr: string;
  promptEn: string;
...
function publicItem(item: PrivateItem, attemptId: string) {
  const safe: Record<string, unknown> = { ...item };
  for (const privateKey of ['answer', 'modelAnswer', 'modelAnswers', 'requiredTerms', 'pairs', 'groups', 'blanks', 'feedbackAr', 'feedbackEn']) {
  ...
  if (item.type === 'match') {
    ...
    safe.left = pairs.map((pair) => pair[0]);
    safe.right = shuffle(pairs.map((pair) => pair[1]), `${attemptId}-${item.id}-pairs`);
  }
  if (item.type === 'familySort') {
    const groups = item.groups ?? {};
    safe.groupLabels = Object.keys(groups);
    safe.values = shuffle(Object.values(groups).flat(), `${attemptId}-${item.id}-groups`);
  }
...
function modelFor(item: PrivateItem) {
  ...
  if (item.type === 'match') return Object.fromEntries((item.pairs ?? []).map((pair) => [pair[0], pair[1]] as const));
  if (item.type === 'familySort') return item.groups;
  if (item.type === 'cloze') return (item.blanks ?? []).map((blank) => blank.answer);
...
function gradeItem(item: PrivateItem, response: unknown) {
  ...
  if (item.type === 'match') {
    ...
    correct = (item.pairs ?? []).every((pair) => equalValue(data[pair[0]], pair[1]));
    ...
    correct = stringArray(response).length === blanks.length
      && blanks.every((blank, index) => equalValue(stringArray(response)[index], blank.answer));
