...
export default function DarijaLessonPage({ params }: { params: Promise<{ lessonId: string }> }) {
  ...
  useEffect(() => {
    let active = true;
    void params.then(({ lessonId: id }) => {
      if (!active) return;
      setLessonId(Number(id));
  ...
  if (!unlocked) return <main className="shell"><section className="locked-page"><h1>هذا الدرس مقفل 🔒</h1><p>أكمل الدرس السابق أولًا لفتحه.</p><Link className="button" href="/darija-lessons">العودة إلى الدروس</Link></section></main>;
  return <main className="shell darija-page"><Link className="back-link" href="/darija-lessons">← كل دروس الدارجة</Link><section className="lesson-view"><div className="eyebrow">الدرس {lesson.id} من 30</div><h1>{lesson.title}</h1><p>{lesson.description}</p>{lesson.videoUrl ? <div className="video-frame"><iframe src={lesson.videoUrl} title={lesson.title} allowFullScreen /></div> : <div className="video-placeholder">ضع رابط فيديو يوتيوب هنا لاحقًا<br/><small>مثال: https://www.youtube.com/embed/VIDEO_ID</small></div>}<div className="lesson-questions"><h2>أسئلة الفيديو</h2>{lesson.questions.map((question, index) => <div className="darija-question" key={question.id}><strong>{index + 1}. {question.prompt}</strong>{question.options.map((option, optionIndex) => <button className={selected[question.id] === optionIndex ? 'selected' : ''} key={option} onClick={() => { setSelected((current) => ({...current, [question.id]: optionIndex})); setMessage(''); }}>{option}</button>)}</div>)}<button className="primary-button" disabled={!allAnswered} onClick={finish}>إرسال الإجابات وفتح الدرس التالي</button>{message && <p className="lesson-message">{message}</p>}</div></section></main>;
}
