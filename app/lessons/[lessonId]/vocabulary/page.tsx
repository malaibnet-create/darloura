...
export default function LessonVocabularyPage() {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      try {
        ...
        const storedReview = savedReview ? JSON.parse(savedReview) as number[] : [];
        setReview(Array.from(new Set([...storedReview, ...lessonOneVocabulary.filter(item => isInReview(`a1-l1-vocabulary-${item.id}`)).map(item => item.id)])));
  ...
  return <main className="shell vocabulary-page">
    <header className="topbar"><Link className="brand" href="/dashboard"><span className="brand-mark">ع</span><span>Dar<span>Lugha</span></span></Link><Link className="link" href="/levels/A1?lesson=1">← Lesson sections</Link></header>
    ...
    <div className="status-legend" dir="ltr"><span><i className="status-dot new" /> New</span><span><i className="status-dot learning" /> Learning</span><span><i className="status-dot mastered" /> Mastered</span><Link href="/review">🔖 {review.length} in review</Link></div>
