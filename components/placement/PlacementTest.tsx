...
export default function PlacementTest() {
  ...
  async function startRecording() {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          setResponses((current) => ({
            ...current,
            [question.id]: { questionId: question.id, recordingPath: path },
          }));
          setError('');
        } catch (uploadError) {
          setError(uploadError instanceof Error
            ? uploadError.message
            : 'Recording upload failed. Check the placement-recordings bucket and its policy.');
        } finally {
          setUploading(false);
        }
      };
      ...
      setError('Microphone permission was denied or is unavailable. You can try again without refreshing the page.');
    }
  }
      ...
      <section className="placement-card">
        ...
        {question.type === 'speaking' ? (
          <div className="recording-box">
            <p>When you are ready, allow your browser to use the microphone.</p>
            {recording ? (
              <button type="button" className="record-button recording" onClick={stopRecording}>
                Stop recording ({recordingSeconds}s)
              </button>
            ) : (
              <button type="button" className="record-button" disabled={uploading} onClick={startRecording}>
                {uploading ? 'Uploading recording…' : 'Start recording'}
              </button>
            )}
            {response?.recordingPath && response.recordingPath !== 'pending-upload' && !recording && (
              <span>Your recording was uploaded successfully. You may record it again.</span>
            )}
          </div>
        ) : null}
        ...
        <div className="placement-actions">
          <button type="button" className="secondary-button" disabled={index === 0 || submitting} onClick={() => setIndex((current) => current - 1)}>Back</button>
          {index < placementQuestions.length - 1 ? (
            <button type="button" className="primary-button" disabled={!answered || uploading || submitting} onClick={() => setIndex((current) => current + 1)}>Next</button>
          ) : (
            <button type="button" className="primary-button" disabled={!answered || uploading || submitting} onClick={submit}>{submitting ? 'Submitting…' : 'Submit assessment'}</button>
          )}
        </div>
      </section>
    </main>
