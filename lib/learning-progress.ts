import { setActiveLearningUser, userStorage as localStorage } from './user-scoped-storage.mjs';
...
export type ProgressSaveResult = {
  localSaved: boolean;
  cloudSaved: boolean;
  reason?: 'storage-unavailable' | 'auth-unavailable' | 'cloud-error';
  error?: string;
};
...
async function cloudClientWithUser() {
  ...
  try {
    ...
    if (user) setActiveLearningUser(user.id);
...
async function syncSectionToCloud(level: LearningLevel, lesson: number, section: LearningSection, status: SectionState, href = ''): Promise<ProgressSaveResult> {
  const cloud = await cloudClientWithUser();
  if (!cloud) return { localSaved: true, cloudSaved: false, reason: 'auth-unavailable' };
  try {
    const { error } = await cloud.client.from('learning_section_progress').upsert({
    ...
    if (error) return { localSaved: true, cloudSaved: false, reason: 'cloud-error', error: error.message };
    ...
    return {
      ...
      cloudSaved: false,
      reason: 'cloud-error',
      error: error instanceof Error ? error.message : 'Unknown progress sync error',
    };
  }
}
...
export async function markSectionComplete(level: LearningLevel, lesson: number, section: LearningSection, href?: string): Promise<ProgressSaveResult> {
  if (!canUseStorage()) return { localSaved: false, cloudSaved: false, reason: 'storage-unavailable' };
  const states = getSectionStates(level, lesson);
  states[section] = 'completed';
  saveStates(level, lesson, states);
  rememberLearningLocation(level, lesson, section, href);
  return syncSectionToCloud(level, lesson, section, 'completed', href);
}
...
export async function hydrateLearningDataFromCloud() {
  ...
  const [sectionResponse, reviewResponse] = await Promise.all([
  ...
  if (sectionResponse.error || reviewResponse.error) return false;
  const sectionRows = sectionResponse.data;
  ...
  const progressUploads: Promise<ProgressSaveResult>[] = [];
  for (const { level, lesson } of touched.values()) {
    const states = getSectionStates(level, lesson);
    for (const section of REQUIRED[level]) {
      if (states[section] !== 'not-started') progressUploads.push(syncSectionToCloud(level, lesson, section, states[section]));
    }
  }
  await Promise.all(progressUploads);
  await Promise.all(review.map((item) => syncReviewToCloud(item)));
