export function readRealtimeEvent(event) {
  ...
  if (event.type === 'response.created' || event.type === 'response.output_audio.delta' || event.type === 'response.audio.delta' || event.type === 'response.output_audio_transcript.delta' || event.type === 'response.audio_transcript.delta') {
    return { status: 'facilitator-speaking' };
  }
  if (event.type === 'response.done') {
    if (event.response?.status === 'failed') return { status: 'error', error: event.response?.status_details?.error?.message || 'تعذر إكمال رد الأستاذ.' };
    return { status: 'learner-turn' };
  }
  if (event.type === 'response.cancelled') return { status: 'learner-turn' };
